...
func targetPermissionsAccessModelToApi(ctx context.Context, s *types.Set) (*[]*buddy.TargetResourcePermission, diag.Diagnostics) {
	var epam []targetPermissionsAccessModel
	diags := s.ElementsAs(ctx, &epam, false)
	targetResourcePermissions := make([]*buddy.TargetResourcePermission, len(epam))
	for i, v := range epam {
		erp := &buddy.TargetResourcePermission{}
		...
		targetResourcePermissions[i] = erp
	}
...
func TargetPermissionsModelToApi(ctx context.Context, s *types.Set) (*buddy.TargetPermissions, diag.Diagnostics) {
	...
	if len(p) != 1 {
		diags.Append(diag.NewErrorDiagnostic("Wrong target permissions settings", "There should be only one environment permissions entry"))
		return nil, diags
	}
	pp := p[0]
