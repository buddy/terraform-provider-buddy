...
var (
	_ datasource.DataSource              = &variablesSource{}
	_ datasource.DataSourceWithConfigure = &variablesSource{}
)

func NewVariablesSource() datasource.DataSource {
...
type variablesSourceModel struct {
...
func (s *variablesSourceModel) loadAPI(ctx context.Context, domain string, variables *[]*buddy.Variable) diag.Diagnostics {
	s.ID = types.StringValue(util.UniqueString())
	s.Domain = types.StringValue(domain)
	v, d := util.VariablesModelFromApi(ctx, variables)
	s.Variables = v
	return d
}

func (s *variablesSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_variables"
}

func (s *variablesSource) Configure(_ context.Context, req datasource.ConfigureRequest, _ *datasource.ConfigureResponse) {
		...
		Attributes: map[string]schema.Attribute{
			...
			"variables": schema.SetNestedAttribute{
				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: util.SourceVariableModelAttributes(),
				},
			},
		},
	}
...
func (s *variablesSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data *variablesSourceModel
