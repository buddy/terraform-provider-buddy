...
test_dev:
	...
	go clean -testcache
	TF_ACC=1 TF_LOG=${TF_LOG} BUDDY_TOKEN=${BUDDY_TOKEN} BUDDY_BASE_URL=https://api.dev.io BUDDY_INSECURE=true go test $(TEST) -v ${TESTNAME} -timeout 60m

test:
	go clean -testcache
	TF_ACC=1 TF_LOG=${TF_LOG} BUDDY_TOKEN=${BUDDY_TOKEN} BUDDY_BASE_URL=${BUDDY_BASE_URL} BUDDY_INSECURE=${BUDDY_INSECURE} go test $(TEST) -v ${TESTNAME} -timeout 60m
